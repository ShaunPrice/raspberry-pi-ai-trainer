...
Automated tests cover bundle tampering/traversal/symlinks/duplicates, cancellation, wrong hardware target/runtime, upload integrity, release activation/rollback and a real local standalone-helper subprocess. Hardware checks are mocked in tests and are clearly fixtures. Tested on Raspberry Pi 5 (8GB) with Raspberry Pi AI HAT+ 13 TOPS (Hailo-8L) and Raspberry Pi AI HAT+ 26 TOPS (Hailo-8). Both completed 64 synthetic inference frames. Full deployment-helper release activation, cancellation and rollback remain unverified on hardware. Hailo-10H hardware testing remains pending.
