...
No live Pi, licensed Hailo compilation, configured remote SSH worker, or real Raspberry Pi OS boot has been verified. Windows/Linux package builds are configured but have not run. The image worker does not resize partitions or install the Hailo runtime: use a base image with the matching packages and enough free filesystem space. Imager preparation/opening never selects or writes a disk.
